...
typedef struct _transposer {
    t_object x_obj;
    t_outlet *x_out;
    double *FFTIn;        // real-valued input buffer
    fftw_complex *FFTOut; // complex FFT output buffer (only 0..N/2 unique bins)
    fftw_plan FFTPlan;    // forward FFT plan (real -> complex)
    fftw_plan IFFTPlan;   // inverse FFT plan (complex -> real)
...
static t_int *transposer_perform(t_int *w) {
    ...
    fftw_complex *newFFT = (fftw_complex *)fftw_alloc_complex(halfSize + 1);
    ...
    */
    ...
    // Normalize the output (FFTW does not normalize automatically).
    for (int i = 0; i < n; i++) {
...
static void transposer_pitch(t_transposer *x, t_floatarg f) { x->pitchScalar = f; }

// Message to set the frequency offset (Hz).
static void transposer_freqshift(t_transposer *x, t_floatarg f) { x->freqShift = f; }

// Message to set the clip flag (nonzero to enable clipping).
static void transposer_clip(t_transposer *x, t_floatarg f) { x->clip = (f != 0); }
...
static void *transposer_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    x->FFTIn = (double *)fftw_alloc_real(x->fftSize);
    if (!x->FFTIn) {
        pd_error(x, "[transposer~] fftw_alloc_real failed");
        return NULL;
    }
    int halfSize = x->fftSize / 2;
    x->FFTOut = (fftw_complex *)fftw_alloc_complex(halfSize + 1);
    if (!x->FFTOut) {
        fftw_free(x->FFTIn);
        pd_error(x, "[transposer~] fftw_alloc_complex failed");
        return NULL;
    }
    // Create FFTW plans for forward (real→complex) and inverse (complex→real) transforms.
...
void transposer_tilde_setup(void) {
    transposer_class =
        class_new(gensym("transposer~"), (t_newmethod)transposer_new, (t_method)transposer_free,
                  sizeof(t_transposer), CLASS_DEFAULT, A_GIMME, 0);
    ...
    class_addmethod(transposer_class, (t_method)transposer_freqshift, gensym("freqshift"), A_FLOAT,
                    0);
