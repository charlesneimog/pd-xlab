...
static void euclidean_bang(EntropyObj *x) {
    ...
    if (!Arr1 || !Arr2) {
        pd_error(x, "[x.renyi] couldn't find some table");
        return;
    }
    t_word *Arr1Vec;
    int Arr1Size;
    ...
    if (Arr1Size != Arr2Size) {
        pd_error(x, "[x.renyi] tables must have the same size");
        return;
    }
    if (x->Normalize) {
        float Sum1 = 0.0;
...
static void euclidean_list(EntropyObj *x, t_symbol *s, int argc, t_atom *argv) {
    ...
    if (size != argc) {
        pd_error(x, "[x.euclidean] list must have the same size");
        return;
    }
...
// ─────────────────────────────────────
extern "C" void setup_x0x2eeuclidean(void) {
    Euclidean = class_new(gensym("x.euclidean"), (t_newmethod)euclidean_new, 0, sizeof(EntropyObj),
                          0, A_GIMME, 0);
    class_addcreator((t_newmethod)euclidean_new, gensym("x.euclidean~"), A_GIMME, 0);
