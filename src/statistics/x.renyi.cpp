...
static void *renyi_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    if (argc == 2) {
        ...
        if (argv[0].a_type != A_SYMBOL || argv[1].a_type != A_SYMBOL) {
            pd_error(x, "[x.renyi~] arg1 and arg2 must be symbols");
            return NULL;
        }
...
// ─────────────────────────────────────
extern "C" void setup_x0x2erenyi(void) {
    renyi_class =
        class_new(gensym("x.renyi"), (t_newmethod)renyi_new, 0, sizeof(renyi), 0, A_GIMME, 0);
    class_addcreator((t_newmethod)renyi_new, gensym("x.renyi~"), A_GIMME, 0);
