...
static void kalman_setnoise(t_kalman *x, t_float f) {
    if ((float)f <= 0) {
        pd_error(x, "[x.kalman] noise cannot be 0 or less");
    } else {
        post("[x.kalman] noise covariance set to %f", f);
...
static void kalman_setinit(t_kalman *x, t_float f) {
    post("[x.kalman] initial val set to %f", f);
    x->init_value = f;
}
...
static void kalman_setiterations(t_kalman *x, t_float f) {
    ...
    if (iter < 1) {
        iter = 1;
        pd_error(x, "[x.kalman] minimum is 1 iteration");
    }
    if (iter > MAX_ITERATIONS) {
        iter = MAX_ITERATIONS;
        pd_error(x, "[x.kalman] exceeded maximum of %d iterations", MAX_ITERATIONS);
    }
    post("[x.kalman] number of iterations set to %d", iter);
    x->iterations = f;
}
...
static void kalman_setanalyze(t_kalman *x, t_float f) {
    if (f > 0) {
        ...
        x->sum = 0;
        x->sumsquares = 0;
        post("[x.kalman] analyzing input for optimal coefficients");
    } else {
        ...
        post("[x.kalman] analyze mode off");
        post("[x.kalman] mean: %f, standard deviation: %f, noise "
             "covariance: %f",
...
// ─────────────────────────────────────
extern "C" void setup_x0x2ekalman(void) {
    kalman_class = class_new(gensym("x.kalman"), (t_newmethod)kalman_new, (t_method)kalman_free,
