...
static void switch_control_dsp(switch_control *x, t_signal **sp) {
    ...
    if (sp[0]->s_n > x->x_allocforvs) {
        ...
        if (!xx) {
            pd_error(0, "x.switch.control~: out of memory");
            return;
        }
        x->x_buf = (t_sample *)xx;
...
static void *switch_control_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    if (argc < 3) {
        pd_error(x, "[x.switch.control~] usage: silence_threshold delay_ms period_samples");
        return NULL;
    }
...
// ─────────────────────────────────────
extern "C" void setup_x0x2eswitch0x2econtrol_tilde(void) {
    switch_control_class =
        class_new(gensym("x.switch.control~"), (t_newmethod)switch_control_new,
