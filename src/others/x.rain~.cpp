...
struct TextureBand {
    double lo, hi, a, b, normalization;
};
constexpr int TEXTURE_BANDS = 6;
// Bed envelope time constants: 95% of a transition takes about 3 * tau.
constexpr double BED_ATTACK_SECONDS = 1.0, BED_RELEASE_SECONDS = 2.0;
...
// ─────────────────────────────────────
// Control ramps advance once per rain_perform call, in seconds rather than
// ticks, so changing sample rate or DSP block size preserves the ramp duration.
struct RainRamp {
    double current, target, remaining;
    void reset(double value) {
        current = target = value;
        remaining = 0;
    }
    void aim(double value, double seconds) {
        if (value != target) {
            target = value;
            remaining = seconds;
        }
    ...
    void advance(double seconds) {
        if (remaining <= 0)
            return;
        if (seconds >= remaining - 1e-12) {
            current = target;
            remaining = 0;
        } else {
            current += (target - current) * (seconds / remaining);
            remaining -= seconds;
        }
    }
};

#define RAIN_CONTROLS(X)                                                                           \
    X(density)                                                                                     \
    X(radius_min)                                                                                  \
    X(radius_max)                                                                                  \
    X(inner)                                                                                       \
...
struct RainControls {
#define RAIN_DECLARE(name) RainRamp name;
    RAIN_CONTROLS(RAIN_DECLARE)
#undef RAIN_DECLARE
};
...
struct t_rain_tilde {
    ...
    t_outlet *out;
    t_outlet *drop_out;
    t_outlet *drop_amplitude_out;
    t_clock *drop_clock;
    t_float drop_amplitude;
    RainControls smoothed;
    double ramp_seconds;
    bool smoothing_ready;
    RainRamp model_mix, surface_mix, material_mix, cdf_mix, water_mix;
    RainMaterial material_from, material_live;
    double cdf_from[DROP_BINS + 1], cdf_live[DROP_BINS + 1];
    double water_from[MAX_WATER], water_live[MAX_WATER];
    int water_from_size, water_live_size;
    ...
    bool water_surface;
    uint32_t rng, texture_rng, bed_rng;
    double bed_envelope;
    bool hybrid;
    int material;
...
// Numeric input fields remain targets. Only the DSP copies below are used
// for synthesis; retargeting continues from the current interpolated value.
static void rain_advance_controls(t_rain_tilde *x, int frames) {
    const double seconds = frames / x->sample_rate;
#define RAIN_ADVANCE(name)                                                                         \
    x->smoothed.name.aim(x->name, x->ramp_seconds);                                                \
    x->smoothed.name.advance(seconds);
    RAIN_CONTROLS(RAIN_ADVANCE)
#undef RAIN_ADVANCE
    x->model_mix.aim(x->hybrid ? 1 : 0, x->ramp_seconds);
    x->surface_mix.aim(x->water_surface ? 1 : 0, x->ramp_seconds);
    x->model_mix.advance(seconds);
    x->surface_mix.advance(seconds);
    x->hybrid_mix = x->model_mix.current;
    x->impact_smooth = x->smoothed.impact.current;
    x->splash_smooth = x->smoothed.splash.current;
    x->tone_smooth = x->smoothed.tone.current;
    x->plinks_smooth = x->smoothed.plinks.current;
    x->bed_smooth = x->smoothed.bed.current;
    x->brightness_smooth = x->smoothed.brightness.current;
    x->intensity_smooth = std::sqrt(x->smoothed.density.current / 800);

    if (x->material_mix.remaining > 0) {
        x->material_mix.advance(seconds);
        const double t = x->material_mix.current;
        const auto &from = x->material_from;
        const auto &to = RAIN_MATERIALS[x->material];
        auto &live = x->material_live;
#define RAIN_MATERIAL_BLEND(name) live.name = from.name + t * (to.name - from.name);
        RAIN_MATERIAL_BLEND(damping)
        RAIN_MATERIAL_BLEND(tone_gain)
        RAIN_MATERIAL_BLEND(splash_gain)
        RAIN_MATERIAL_BLEND(splash_seconds)
        RAIN_MATERIAL_BLEND(attack_seconds)
        RAIN_MATERIAL_BLEND(cutoff)
        RAIN_MATERIAL_BLEND(highpass)
        RAIN_MATERIAL_BLEND(crinkle)
        RAIN_MATERIAL_BLEND(impact_gain)
#undef RAIN_MATERIAL_BLEND
        for (int i = 0; i < 3; ++i)
            live.frequencies[i] =
                from.frequencies[i] + t * (to.frequencies[i] - from.frequencies[i]);
        for (int i = 0; i < TEXTURE_BANDS; ++i)
            live.bed_weights[i] =
                from.bed_weights[i] + t * (to.bed_weights[i] - from.bed_weights[i]);
    }
    if (x->cdf_mix.remaining > 0) {
        x->cdf_mix.advance(seconds);
        for (int i = 0; i <= DROP_BINS; ++i)
            x->cdf_live[i] =
                x->cdf_from[i] + x->cdf_mix.current * (x->drop_cdf[i] - x->cdf_from[i]);
    }
    if (x->water_mix.remaining > 0) {
        x->water_mix.advance(seconds);
        x->water_live_size = x->water_mix.remaining > 0
                                 ? std::max(x->water_from_size, x->water_size)
                                 : x->water_size;
        for (int i = 0; i < std::max(x->water_from_size, x->water_size); ++i)
            x->water_live[i] =
                x->water_from[i] + x->water_mix.current * (x->water_delta[i] - x->water_from[i]);
    }
}
...
static void rain_prepare_drops(t_rain_tilde *x) {
    std::copy_n(x->cdf_live, DROP_BINS + 1, x->cdf_from);
    ...
    // Even a zero-width/zero-flux endpoint needs a normalized CDF while
    // crossfading. Otherwise upper_bound could sample beyond its last bin.
    for (int i = 1; i <= DROP_BINS; ++i)
        x->drop_cdf[i] = total > 0 ? x->drop_cdf[i] / total : double(i) / DROP_BINS;
    rain_update_density(x);
    if (x->smoothing_ready) {
        x->cdf_mix.reset(0);
        x->cdf_mix.aim(1, x->ramp_seconds);
    } else {
        std::copy_n(x->drop_cdf, DROP_BINS + 1, x->cdf_live);
        x->cdf_mix.reset(1);
    }
}
...
static double rain_drop_radius(t_rain_tilde *x) {
    ...
    if (x->smoothed.radius_min.current == x->smoothed.radius_max.current ||
        x->cdf_live[DROP_BINS] == 0)
        return x->smoothed.radius_min.current;
    const double *upper = std::upper_bound(x->cdf_live, x->cdf_live + DROP_BINS + 1, u);
    int bin = std::min(DROP_BINS - 1, int(upper - x->cdf_live) - 1);
    double fraction = (u - x->cdf_live[bin]) / (x->cdf_live[bin + 1] - x->cdf_live[bin]);
    // Invert the exponential within the bin (the speed is held at its midpoint).
    double step =
        2000 * (x->smoothed.radius_max.current - x->smoothed.radius_min.current) / DROP_BINS;
    double offset = -std::log1p(fraction * std::expm1(-x->smoothed.drop_lambda.current * step)) /
                    x->smoothed.drop_lambda.current;
    return std::min(x->smoothed.radius_max.current,
                    x->smoothed.radius_min.current + (bin * step + offset) / 2000);
}
...
// ─────────────────────────────────────
static double rain_background(t_rain_tilde *x) {
    // Continuous stationary noise: no random amplitude modulation or drop gate.
    // Its own PRNG/filter history keeps the bed independent of individual drops.
    double result = 0;
    for (int j = 0; j < TEXTURE_BANDS; ++j) {
        auto &b = x->texture[j];
        double noise = 2 * rain_texture_random(x->bed_rng) - 1;
        ...
        weight *= x->material_live.bed_weights[j];
        result += (b.hi - b.lo) * b.normalization * weight;
    }
    return result * 0.07;
}
...
static int rain_kernel(double *out, double fraction, double a, double x0, const t_rain_tilde *x) {
    double Rs = std::hypot(x0 - a, x->smoothed.height.current),
           Rl = std::hypot(x0 + a, x->smoothed.height.current);
    double duration = 4 * x0 * a / ((Rl + Rs) * x->smoothed.c.current);
    ...
    for (int i = 0; i < count; ++i)
        out[i] = rain_bin((i - fraction) / x->sample_rate, (i + 1 - fraction) / x->sample_rate, a,
                          x0, x->smoothed.height.current, x->smoothed.rho.current,
                          x->smoothed.c.current, x->sample_rate);
    return count;
}
...
static bool rain_prepare_water(t_rain_tilde *x) {
    std::copy_n(x->water_live, MAX_WATER, x->water_from);
    x->water_from_size = x->water_live_size;
    std::fill_n(x->water_delta, MAX_WATER, 0.0);
    x->water_mix.reset(0);
    x->water_mix.aim(1, x->ramp_seconds);
    x->water_size = 0;
...
static void rain_trigger(t_rain_tilde *x, double arrival, bool manual = false) {
    ...
    const double inner = std::max(
        x->smoothed.inner.current,
        std::nextafter(std::max(x->smoothed.radius_max.current, x->smoothed.opening_radius.current),
                       INFINITY));
    const double outer = std::max(inner, x->smoothed.outer.current);
    double x0 = std::sqrt(inner * inner + rain_uniform(x) * (outer * outer - inner * inner));
    ...
    v = RainVoice{};
    const auto &material = x->material_live;
    v.impact_gain = material.impact_gain;
    const bool water_enabled = x->surface_mix.current > 0 && x->water_live_size > 0;
    double hard_time = std::hypot(x0 - a, x->smoothed.height.current) / x->smoothed.c.current;
    double water_time =
        water_enabled
            ? (std::hypot(x0 - x->smoothed.opening_radius.current, x->smoothed.height.current) +
               x->smoothed.cone_length.current) /
                  x->smoothed.c.current
            : hard_time;
    ...
    v.amplitude = x->smoothed.amplitude.current;
    ...
    if (water_enabled) {
        ...
        v.water_size = rain_kernel(v.water, water_arrival - v.water_start,
                                   x->smoothed.opening_radius.current, x0, x);
        v.end = std::max(v.end, v.water_start + v.water_size + x->water_live_size - 1);
    }
    if (!v.hard_size || (water_enabled && !v.water_size)) {
        ++x->dropped;
        return;
    }
    ...
    if (x->hybrid_mix > 0 &&
        (manual || rain_texture_random(x->texture_rng) <
                       std::min(1.0, 800 / std::max(1.0, x->smoothed.density.current)))) {
        ...
        double sparse = 1 - std::min(1.0, x->smoothed.density.current / 150);
        ...
        double decay = x->smoothed.resonance.current * 0.001 * material.damping *
        ...
        if (x->smoothed.plinks.current > 0 && rain_texture_random(x->texture_rng) < 0.08) {
            v.plink_start =
                v.detail_start +
...
static t_int *rain_perform(t_int *w) {
    ...
    rain_advance_controls(x, n);
    bool drop_played = false;
    double drop_peak = 0;
    if (x->hybrid) {
        // Here I randomize the x.rhythm with exponential arrival intervals
        ...
        if (x->smoothed.density.current > 0) {
            if (x->arrival_hazard < 0)
                x->arrival_hazard = -std::log1p(-rain_uniform(x));
            double duration = n / x->sample_rate, elapsed = 0;
            while (x->arrival_hazard < x->smoothed.density.current * (duration - elapsed)) {
                elapsed += x->arrival_hazard / x->smoothed.density.current;
                rain_trigger(x, elapsed * x->sample_rate);
                x->arrival_hazard = -std::log1p(-rain_uniform(x));
            }
            x->arrival_hazard -= x->smoothed.density.current * (duration - elapsed);
        }
    } else {
        ...
        x->count_fraction += x->smoothed.density.current * n / x->sample_rate;
    ...
    // Smooth the combined bed level, not just its individual multipliers.
    // A short density/bed gap must not empty the background in 100 ms.
    const double bed_target = x->hybrid_mix * x->bed_smooth * x->intensity_smooth;
    const double bed_begin = x->bed_envelope;
    const double bed_tau = bed_target > bed_begin ? BED_ATTACK_SECONDS : BED_RELEASE_SECONDS;
    const double bed_follow = -std::expm1(-n / (bed_tau * x->sample_rate));
    x->bed_envelope += bed_follow * (bed_target - x->bed_envelope);
    if (bed_target == 0 && x->bed_envelope < 1e-12)
        x->bed_envelope = 0;
    const double bed_step = (x->bed_envelope - bed_begin) / n;
    for (int i = 0; i < n; ++i) {
        double l = 0;
        for (int j = 0; j < x->active;) {
            RainVoice &v = x->voices[j];
            if (v.water_size && x->water_live_size)
                v.end = std::max(v.end, v.water_start + v.water_size + x->water_live_size - 1);
            // Track only drops whose first layer starts in this block.
            // v.age - i is this voice's age at the beginning of the block;
            // later water arrivals and tails from older drops are excluded.
            const int first = v.water_size ? std::min(v.hard_start, v.water_start) : v.hard_start;
            const bool new_drop = x->drop_out && first >= v.age - i && first <= v.age;
            drop_played = drop_played || new_drop;
            double p = 0;
            ...
            if (water >= 0 && v.water_size) {
                int begin = std::max(0, water - x->water_live_size + 1);
                int end = std::min(v.water_size - 1, water);
                for (int k = begin; k <= end; ++k)
                    p += x->surface_mix.current * v.water[k] * x->water_live[water - k];
            }
            ...
            p = p * (1 + x->hybrid_mix * (x->impact_smooth * v.size_gain * v.impact_gain - 1)) +
                x->hybrid_mix * detail;
            // Linear absolute peak of the strongest new drop in this block,
            // including output gain, but excluding the bed and other drops.
            if (new_drop)
                drop_peak = std::max(drop_peak, std::fabs(p * x->smoothed.gain.current));
        ...
        // Advance the noise even at zero volume. Interpolate the envelope
        // within this DSP block so the continuous bed has no boundary steps.
        l += (bed_begin + bed_step * (i + 1)) * rain_background(x);
        ...
        left[i] = static_cast<t_sample>(l * x->smoothed.gain.current);
    }
    // Defer Pd messages until after DSP, with at most one per block.
    if (drop_played) {
        x->drop_amplitude = static_cast<t_float>(drop_peak);
        clock_delay(x->drop_clock, 0);
    }
    return w + 4;
}
...
static void rain_rainrate(t_rain_tilde *x, t_floatarg value) {
    if (!std::isfinite(value) || value < 0 || value > 1000) {
        pd_error(x, "x.rain~: rainrate must be 0..1000 mm/hour");
        return;
    }
    const bool changed = x->rain_rate != value;
    x->rain_rate = value;
...
static void rain_collection(t_rain_tilde *x, t_floatarg value) {
    if (!std::isfinite(value) || value < 0 || value > 1000) {
        pd_error(x, "x.rain~: collection must be 0..1000 square meters");
        return;
    }
...
static void rain_bang(t_rain_tilde *x) {
    if (x->pending < MAX_VOICES)
        ...
        pd_error(x, "x.rain~: pending drop capacity exceeded");
}
...
static void rain_radius(t_rain_tilde *x, t_floatarg lo, t_floatarg hi) {
    if (!std::isfinite(lo) || !std::isfinite(hi) || lo <= 0 || hi < lo || hi > 0.02 ||
        hi >= x->inner) {
        pd_error(x, "x.rain~: radius min max in meters: 0 < min <= max <= 0.02, max < inner radius");
        return;
    }
    if (x->radius_min == lo && x->radius_max == hi)
        return;
    x->radius_min = lo;
    x->radius_max = hi;
    rain_prepare_drops(x);
...
static void rain_area(t_rain_tilde *x, t_floatarg inner, t_floatarg outer) {
    if (!std::isfinite(inner) || !std::isfinite(outer) ||
        ...
        pd_error(x,
                 "x.rain~: area inner outer in meters: inner > source radii, inner <= outer <= 1000");
        return;
    }
    x->inner = inner;
    x->outer = outer;
...
static void rain_height(t_rain_tilde *x, t_floatarg value) {
    if (std::isfinite(value) && value > 0 && value <= 1000)
        ...
        pd_error(x, "x.rain~: height must be in (0, 1000] meters");
}
...
static void rain_amplitude(t_rain_tilde *x, t_floatarg value) {
    ...
    if (std::isfinite(value) && value >= 0 && value <= 1000)
        ...
        pd_error(x, "x.rain~: amplitude A(vterm) must be in [0,1000] m/s");
}
...
static void rain_gain(t_rain_tilde *x, t_floatarg value) {
    if (std::isfinite(value) && value >= 0 && value <= 1000000)
        ...
        pd_error(x, "x.rain~: gain must be in [0,1000000]");
}
...
static void rain_surface(t_rain_tilde *x, t_symbol *name) {
    if (!std::strcmp(name->s_name, "hard"))
        ...
        pd_error(x, "x.rain~: use surface hard, or configure cone and pinch before surface water");
}
...
static void rain_cone(t_rain_tilde *x, t_floatarg l, t_floatarg radius, t_floatarg splay) {
    if (!std::isfinite(l) || !std::isfinite(radius) || !std::isfinite(splay) || l <= 0 || l > 1 ||
        ...
        pd_error(x,
                 "x.rain~: cone l radius lambda requires l>0, radius<=0.02 m and s=radius/lambda>l");
        return;
    }
    x->cone_length = l;
    ...
    if (!rain_prepare_water(x)) {
        x->water_surface = false;
        if (x->pinch_size)
            pd_error(x, "x.rain~: cone response exceeds water buffer capacity");
    }
}
...
static void rain_pinch(t_rain_tilde *x, t_symbol *name, t_floatarg rate) {
    ...
    if (!array || !garray_getfloatwords(array, &size, &words) || size < 1 || size > MAX_PINCH ||
        ...
        pd_error(x, "x.rain~: pinch requires a float array of 1..2048 velocity samples and optional "
                    "sample rate");
        return;
    }
    double input_rate = rate > 0 ? rate : x->sample_rate;
    if (input_rate < 1000 || input_rate > 384000) {
        pd_error(x, "x.rain~: pinch sample rate must be 1000..384000 Hz");
        return;
    }
    for (int i = 0; i < size; ++i)
        if (!std::isfinite(words[i].w_float) || std::fabs(words[i].w_float) > 1000) {
            pd_error(x, "x.rain~: pinch samples must be finite velocities within +/-1000 m/s");
            return;
        }
    x->pinch_size = size;
    ...
    if (!rain_prepare_water(x)) {
        x->water_surface = false;
        pd_error(x, "x.rain~: configure cone; waveform plus tail must fit the water buffer");
    }
}
...
static void rain_air(t_rain_tilde *x, t_floatarg rho, t_floatarg c) {
    if (!std::isfinite(rho) || !std::isfinite(c) || rho <= 0 || rho > 10 || c < 250 || c > 450) {
        pd_error(x, "x.rain~: air rho c requires 0<rho<=10 kg/m^3 and 250<=c<=450 m/s");
        return;
    }
    x->rho = rho;
    x->c = c;
    if (x->pinch_size && !rain_prepare_water(x)) {
        x->water_surface = false;
        pd_error(x, "x.rain~: water response exceeds buffer capacity");
    }
}

// ─────────────────────────────────────
static void rain_status(t_rain_tilde *x) {
    post("x.rain~: DAFx 2004 Eq. 1/2/3; %d active; %llu capacity-dropped events", x->active,
         static_cast<unsigned long long>(x->dropped));
    post("x.rain~: Marshall-Palmer %.6g mm/hour; collection %.6g m^2; %.6g drops/s (%s); "
...
static void rain_model(t_rain_tilde *x, t_symbol *name) {
    if (!std::strcmp(name->s_name, "hybrid"))
        ...
        pd_error(x, "x.rain~: model must be hybrid or paper");
}
...
static void rain_material(t_rain_tilde *x, t_symbol *name) {
    ...
    for (int i = 0; i < MATERIAL_COUNT; ++i)
        if (!std::strcmp(selected, RAIN_MATERIALS[i].name)) {
            if (x->material != i) {
                x->material_from = x->material_live;
                x->material_mix.reset(0);
                x->material_mix.aim(1, x->ramp_seconds);
                x->material = i;
            }
            return;
        }
    pd_error(x, "x.rain~: material must be plastic, roof, wood, dirt, asphalt, ground or leaf");
}
...
// ─────────────────────────────────────
// [smooth 100( sets the duration of subsequent target changes (milliseconds).
static void rain_smooth(t_rain_tilde *x, t_floatarg milliseconds) {
    if (std::isfinite(milliseconds) && milliseconds >= 1 && milliseconds <= 10000)
        x->ramp_seconds = milliseconds / 1000.0;
    else
        pd_error(x, "x.rain~: smooth expects 1..10000 milliseconds");
}

static void rain_legacy(t_rain_tilde *x, t_symbol *, int, t_atom *) {
    pd_error(
        x, "x.rain~: unsupported control; use impact/splash/tone/plinks/bed or radius, area, height, "
...
static void rain_dsp(t_rain_tilde *x, t_signal **sp) {
    if (x->drop_clock)
        clock_unset(x->drop_clock);
    double sr = sp[0]->s_sr;
    if (!std::isfinite(sr) || sr < 1000 || sr > 384000) {
        pd_error(x, "x.rain~: supported sample rates are 1000..384000 Hz");
        return;
    }
    if (sr != x->sample_rate) {
        rain_clear(x);
        ...
        if (x->pinch_size && !rain_prepare_water(x)) {
            x->water_surface = false;
            pd_error(x, "x.rain~: resampled water response exceeds buffer capacity");
        }
    }
...
// ─────────────────────────────────────
static void rain_drop_tick(t_rain_tilde *x) {
    // Right-to-left: update the amplitude before triggering downstream logic.
    outlet_float(x->drop_amplitude_out, x->drop_amplitude);
    outlet_bang(x->drop_out);
}

static void rain_free(t_rain_tilde *x) {
    if (x->drop_clock)
        clock_free(x->drop_clock);
}

// [x.rain~], [x.rain~ 100], [x.rain~ -bang], [x.rain~ 100 -bang].
// Missing or zero density starts silent; rainrate enables automatic density.
// -bang adds bang and amplitude outlets after the existing signal outlet.
// Amplitude is the strongest new drop's linear peak in its onset DSP block.
static void *rain_new(t_symbol *, int argc, t_atom *argv) {
    t_float density = 0;
    bool has_density = false, bang_outlet = false;
    for (int i = 0; i < argc; ++i) {
        if (argv[i].a_type == A_SYMBOL && atom_getsymbol(argv + i) == gensym("-bang") &&
            !bang_outlet) {
            bang_outlet = true;
        } else if (argv[i].a_type == A_FLOAT && !has_density &&
                   std::isfinite(atom_getfloat(argv + i))) {
            density = atom_getfloat(argv + i);
            has_density = true;
        } else {
            pd_error(nullptr, "x.rain~: expected an optional density and -bang flag");
            return nullptr;
        }
    }
    auto *x = reinterpret_cast<t_rain_tilde *>(pd_new(rain_tilde_class));
    x->drop_out = nullptr;
    x->drop_amplitude_out = nullptr;
    x->drop_clock = nullptr;
    x->drop_amplitude = 0;
    x->ramp_seconds = 0.1;
    x->smoothing_ready = false;
    ...
    // Prepare the default drop-size distribution, then apply the requested
    // density (zero when omitted) before initializing the DSP control ramps.
    rain_rainrate(x, 5);
    ...
    rain_texture_prepare(x);
#define RAIN_INITIALIZE(name) x->smoothed.name.reset(x->name);
    RAIN_CONTROLS(RAIN_INITIALIZE)
#undef RAIN_INITIALIZE
    x->model_mix.reset(x->hybrid ? 1 : 0);
    x->surface_mix.reset(0);
    x->material_mix.reset(1);
    x->water_mix.reset(1);
    x->material_from = x->material_live = RAIN_MATERIALS[x->material];
    x->smoothing_ready = true;
    x->out = outlet_new(&x->x_obj, &s_signal);
    if (bang_outlet) {
        x->drop_out = outlet_new(&x->x_obj, &s_bang);
        x->drop_amplitude_out = outlet_new(&x->x_obj, &s_float);
        x->drop_clock = clock_new(x, reinterpret_cast<t_method>(rain_drop_tick));
    }
    return x;
}

// ─────────────────────────────────────
extern "C" void setup_x0x2erain_tilde() {
    rain_tilde_class = class_new(gensym("x.rain~"), reinterpret_cast<t_newmethod>(rain_new),
                                 reinterpret_cast<t_method>(rain_free), sizeof(t_rain_tilde),
                                 CLASS_DEFAULT, A_GIMME, 0);
    class_addbang(rain_tilde_class, reinterpret_cast<t_method>(rain_bang));
    class_addmethod(rain_tilde_class, reinterpret_cast<t_method>(rain_smooth), gensym("smooth"),
                    A_FLOAT, 0);
