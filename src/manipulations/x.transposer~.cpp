...
static void *transposer_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    if (!x->FFTIn) {
        pd_error(x, "[x.transposer~] fftwf_alloc_real failed");
        return NULL;
    }
    int halfSize = x->fftSize / 2;
    ...
    if (!x->FFTOut) {
        fftwf_free(x->FFTIn);
        pd_error(x, "[x.transposer~] fftwf_alloc_complex failed");
        return NULL;
    }
...
extern "C" void setup_x0x2etransposer_tilde(void) {
    transposer_class = class_new(gensym("x.transposer~"),
