...
static void arrayinvert_bang(arrayinvert *x) {
    ...
    if (!(array = (t_garray *)pd_findbyclass(pd_symbol, garray_class))) {
        pd_error(x, "[x.array.invert] array %s not found.", x->arrayname.c_str());
        return;
    } else if (!garray_getfloatwords(array, &vecsize, &vec)) {
        pd_error(x, "[x.array.invert] Bad template for tabwrite '%s'.", x->arrayname.c_str());
        return;
    }
...
// ─────────────────────────────────────
extern "C" void setup_x0x2earray0x2einvert(void) {
    neimog_arrayinvert = class_new(gensym("x.array.invert"), (t_newmethod)arrayinvert_new, 0,
