...
static void arrayappend_float(arrayappend *x, t_float f) {
    ...
    if (!(array = (t_garray *)pd_findbyclass(pd_symbol, garray_class))) {
        pd_error(x, "[x.array.append] Array %s not found.", x->arrayname.c_str());
        return;
    } else if (!garray_getfloatwords(array, &vecsize, &vec)) {
        pd_error(x, "[x.array.append] Bad template for tabwrite '%s'.", x->arrayname.c_str());
        return;
    }

    x->arraysize = garray_npoints(array);
    if (x->arraysize + x->resize_step > NEIMOG_MAXARRAYSIZE) {
        pd_error(x, "[x.array.append] Array too big.");
        x->index = 0;
        return;
    }

    if (x->arraysize - 1 < x->index) {
        pd_error(x, "[x.array.append] Array too small.");
        return;
    }
    ...
    if (x->arraysize - 1 == x->index) {
        ...
        if (!(array = (t_garray *)pd_findbyclass(pd_symbol, garray_class))) {
            pd_error(x, "[x.array.append] a.arrayappend %s not found.", x->arrayname.c_str());
            return;
        }
...
// ─────────────────────────────────────
extern "C" void setup_x0x2earray0x2eappend(void) {
    neimog_arrayappend = class_new(gensym("x.array.append"), (t_newmethod)arrayappend_new, 0,
