...
static void clarinet_tilde_list(t_clarinet_tilde *x, t_symbol *, int argc, t_atom *argv) {
    if (!x->model)
        return;
    if (argc != 2) {
        pd_error(x, "[x.clarinet~] list expects: <midi-pitch> <velocity 0..127>");
        return;
    }
...
static void clarinet_tilde_mode(t_clarinet_tilde *x, t_symbol *, int argc, t_atom *argv) {
    if (!x->model)
        return;
    if (argc != 5) {
        pd_error(x, "[x.clarinet~] mode expects: <index> <ratio> <Q> <residue-re> <residue-im>");
        return;
    }
    ...
    if (!x->model->setMode(index, ratio, q, cre, cim)) {
        pd_error(x, "[x.clarinet~] invalid mode parameters");
    }
}
...
static void clarinet_tilde_print(t_clarinet_tilde *x) {
    if (!x->model)
        return;
    post("x.clarinet~ modal physical model:");
...
static void *clarinet_tilde_new(t_symbol *, int argc, t_atom *argv) {
    ...
    try {
        ...
        pd_error(x, "[x.clarinet~] could not allocate physical model");
    } catch (...) {
        pd_error(x, "[x.clarinet~] unexpected construction error");
    }
...
extern "C" void setup_x0x2eclarinet_tilde(void) {
    clarinet_tilde_class =
        class_new(gensym("x.clarinet~"), reinterpret_cast<t_newmethod>(clarinet_tilde_new),
...
#undef ADD_FLOAT_METHOD
    ...
    post("x.clarinet~: multimodal reed + modal input-impedance physical model");
}
