...
static void besseldrum_do_strike(t_besseldrum_tilde *x, double sx, double sy, double velocity) {
    if (!std::isfinite(sx) || !std::isfinite(sy) || !finite_positive(velocity)) {
        if (!std::isfinite(sx) || !std::isfinite(sy) || !std::isfinite(velocity))
            pd_error(x, "x.besseldrum~: strike values must be finite");
        return;
    }
...
static void besseldrum_strike(t_besseldrum_tilde *x, t_symbol *, int argc, t_atom *argv) {
    ...
    pd_error(x, "x.besseldrum~: strike expects "
                "'strike velocity' or "
                "'strike x y velocity'");
}
...
static void besseldrum_strikepos(t_besseldrum_tilde *x, t_floatarg fx, t_floatarg fy) {
    ...
    if (!std::isfinite(px) || !std::isfinite(py)) {
        pd_error(x, "x.besseldrum~: strikepos values must be finite");
        return;
    }
    project_to_membrane(px, py);
    x->strike_x = px;
...
static void besseldrum_pickup(t_besseldrum_tilde *x, t_floatarg fx, t_floatarg fy) {
    ...
    if (!std::isfinite(px) || !std::isfinite(py)) {
        pd_error(x, "x.besseldrum~: pickup values must be finite");
        return;
    }
    x->pickup_x = px;
    x->pickup_y = py;
...
static void besseldrum_tune(t_besseldrum_tilde *x, t_floatarg f) {
    ...
    if (!finite_positive(value)) {
        pd_error(x, "x.besseldrum~: tune must be a finite value > 0 Hz");
        return;
    }
    ...
    if (!std::isfinite(tension) ||
        ...
        pd_error(x, "x.besseldrum~: tune produced a non-finite tension");
        return;
    }
    if (tension < 0.0L) {
        ...
        if (tension >= -tolerance) {
            ...
            pd_error(x,
                     "x.besseldrum~: %.3f Hz is below the zero-tension bending "
                     "frequency %.3f Hz",
...
static void besseldrum_radius(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_positive(value)) {
        pd_error(x, "x.besseldrum~: radius must be a finite value > 0 meters");
        return;
    }
    x->radius = value;
    besseldrum_update_coefficients(x);
...
static void besseldrum_density(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_positive(value)) {
        pd_error(x, "x.besseldrum~: density must be a finite value > 0 kg/m3");
        return;
    }
    x->density = value;
...
static void besseldrum_thickness(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_positive(value)) {
        pd_error(x, "x.besseldrum~: thickness must be a finite value > 0 meters");
        return;
    }
    x->thickness = value;
    x->material = gensym("custom");
...
static void besseldrum_tension(t_besseldrum_tilde *x, t_floatarg f) {
    ...
    if (!finite_nonnegative(value)) {
        pd_error(x, "x.besseldrum~: tension must be a finite value >= 0 N/m");
        return;
    }
    x->tension = value;
    besseldrum_update_coefficients(x);
...
static void besseldrum_young(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_nonnegative(value)) {
        pd_error(x, "x.besseldrum~: young must be a finite value >= 0 Pa");
        return;
    }
    x->young = value;
    x->material = gensym("custom");
...
static void besseldrum_poisson(t_besseldrum_tilde *x, t_floatarg f) {
    ...
    if (!std::isfinite(value) || value <= -1.0 || value >= 0.5) {
        pd_error(x, "x.besseldrum~: poisson must be finite and satisfy -1 < nu < 0.5");
        return;
    }
    x->poisson = value;
    x->material = gensym("custom");
...
static void besseldrum_loss0(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_nonnegative(value)) {
        pd_error(x, "x.besseldrum~: loss0 must be finite and >= 0");
        return;
    }
    x->loss0 = value;
    x->material = gensym("custom");
...
static void besseldrum_loss1(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_nonnegative(value)) {
        pd_error(x, "x.besseldrum~: loss1 must be finite and >= 0");
        return;
    }
    x->loss1 = value;
    x->material = gensym("custom");
...
static void besseldrum_loss2(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_nonnegative(value)) {
        pd_error(x, "x.besseldrum~: loss2 must be finite and >= 0");
        return;
    }
    x->loss2 = value;
    x->material = gensym("custom");
...
static void besseldrum_material(t_besseldrum_tilde *x, t_symbol *symbol) {
    if (!symbol || !symbol->s_name) {
        pd_error(x, "x.besseldrum~: material expects a preset name");
        return;
    }
    ...
    if (!preset) {
        pd_error(x,
                 "x.besseldrum~: unknown material '%s' "
...
static void besseldrum_gain(t_besseldrum_tilde *x, t_floatarg f) {
    const double value = static_cast<double>(f);
    if (!finite_nonnegative(value)) {
        pd_error(x, "x.besseldrum~: gain must be finite and >= 0");
        return;
    }
    x->gain = value;
}
...
static void besseldrum_modes(t_besseldrum_tilde *x, t_floatarg f) {
    ...
    if (!std::isfinite(value)) {
        pd_error(x, "x.besseldrum~: modes must be finite");
        return;
    }
...
static void *besseldrum_new(t_symbol *, int argc, t_atom *argv) {
    ...
    // Backward-compatible creation form: [x.besseldrum~ fundamental modes].
...
// ─────────────────────────────────────
extern "C" void setup_x0x2ebesseldrum_tilde(void) {
    initialize_mode_table();

    besseldrum_tilde_class =
        class_new(gensym("x.besseldrum~"), reinterpret_cast<t_newmethod>(besseldrum_new),
    ...
    post("x.besseldrum~: circular membrane "
