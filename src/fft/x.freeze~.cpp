...
static void xfreeze_dsp(t_xfreeze *x, t_signal **sp) {
    ...
    if ((x->dspBlockSize != x->fftSize || (x->dspBlockSize % x->hopSize) != 0) &&
        !x->warnedBlockSize) {
        post("[x.freeze~] warning: block size (%d) must equal fftSize (%d) and be divisible by hop "
             "(%d); "
...
static void xfreeze_frames(t_xfreeze *x, t_floatarg f) {
    ...
    if (!xfreeze_alloc_frames(x, n)) {
        pd_error(x, "[x.freeze~] could not allocate frame buffers");
    }
}
...
static void xfreeze_array(t_xfreeze *x, t_symbol *s) {
    ...
    if (!xfreeze_matrix_bind(x, s)) {
        pd_error(x, "[x.freeze~] array '%s' not found or invalid", s->s_name);
    }
}
...
static void *xfreeze_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    if (!xfreeze_init_fft(x, x->fftSize)) {
        pd_error(x, "[x.freeze~] failed to initialize FFT buffers/plans");
        return NULL;
    }

    if (!xfreeze_init_processing(x)) {
        pd_error(x, "[x.freeze~] failed to initialize processing buffers");
        if (x->fftPlan)
            fftw_destroy_plan(x->fftPlan);
        if (x->ifftPlan)
            fftw_destroy_plan(x->ifftPlan);
    ...
    if (!xfreeze_alloc_frames(x, x->numFrames)) {
        pd_error(x, "[x.freeze~] failed to allocate frame buffers");
        if (x->fftPlan)
            fftw_destroy_plan(x->fftPlan);
        if (x->ifftPlan)
            fftw_destroy_plan(x->ifftPlan);
...
// ─────────────────────────────────────
extern "C" void setup_x0x2efreeze_tilde(void) {
    xfreeze_class = class_new(gensym("x.freeze~"), (t_newmethod)xfreeze_new, (t_method)xfreeze_free,
