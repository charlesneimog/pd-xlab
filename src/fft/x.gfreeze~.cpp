...
static void gfreeze_frames(t_gfreeze *x, t_floatarg f) {
    ...
    if (n != x->numFrames) {
        if (!gfreeze_alloc_frames(x, n)) {
            pd_error(x, "[x.gfreeze~] Memory allocation failed for %d frames.", n);
        }
    }
}
...
static void gfreeze_dsp(t_gfreeze *x, t_signal **sp) {
    ...
    if ((x->dspBlockSize != x->fftSize) && !x->warnedBlockSize) {
        post("[x.gfreeze~] warning: Pd block size (%d) must equal fftSize (%d) for optimal "
             "performance.",
...
static void *gfreeze_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    if (!gfreeze_init_fft(x, x->fftSize)) {
        pd_error(x, "[x.gfreeze~] FFT init failed");
        return NULL;
    }
    if (!gfreeze_init_processing(x)) {
        pd_error(x, "[x.gfreeze~] Processing init failed");
        return NULL;
    }
    if (!gfreeze_alloc_frames(x, x->numFrames)) {
        pd_error(x, "[x.gfreeze~] Frame alloc failed");
        return NULL;
    }
...
// ─────────────────────────────────────
extern "C" void setup_x0x2egfreeze_tilde(void) {
    gfreeze_class = class_new(gensym("x.gfreeze~"), (t_newmethod)gfreeze_new, (t_method)gfreeze_free,
