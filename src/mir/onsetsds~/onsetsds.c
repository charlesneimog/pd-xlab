...
float onsetsds_phase_rewrap(float phase) {
    return (phase > MINUSPI && phase < PI)
               ? phase
               : phase + TWOPI * (1.f + floorf((MINUSPI - phase) * INV_TWOPI));
}
...
void onsetsds_init(OnsetsDS *ods, float *odsdata, int fftformat, int odftype, size_t fftsize,
                   unsigned int medspan, float srate) {
    ...
    case ODS_ODF_WPHASE:
        ...
        break;
    case ODS_ODF_MKL:
        ods->odfparam = 0.01; // EPSILON parameter. Brossier recommends 1e-6 but I (ICMC 2007) found
                              // larger vals (e.g 0.01) to work better
...
void onsetsds_loadframe(OnsetsDS *ods, float *fftbuf) {
    ...
    // Conversion to log-domain magnitudes, including re-scaling to aim back at the zero-to-one
    // range. Not well tested yet.
    if (ods->logmags) {
        for (i = 0; i < ods->numbins; i++) {
            ods->curr->bin[i].mag = (log(ods_max(ods->curr->bin[i].mag, ODS_LOG_LOWER_LIMIT)) -
                                     ODS_LOGOF_LOG_LOWER_LIMIT) *
                                    ODS_ABSINVOF_LOGOF_LOG_LOWER_LIMIT;
        }
        ods->curr->dc = (log(ods_max(ods_abs(ods->curr->dc), ODS_LOG_LOWER_LIMIT)) -
                         ODS_LOGOF_LOG_LOWER_LIMIT) *
                        ODS_ABSINVOF_LOGOF_LOG_LOWER_LIMIT;
        ods->curr->nyq = (log(ods_max(ods_abs(ods->curr->nyq), ODS_LOG_LOWER_LIMIT)) -
                          ODS_LOGOF_LOG_LOWER_LIMIT) *
                         ODS_ABSINVOF_LOGOF_LOG_LOWER_LIMIT;
    }
}
...
void onsetsds_odf(OnsetsDS *ods) {
    ...
    case ODS_ODF_RCOMPLEX:
        ...
        // Note: "other" buf is stored in this format:
        // mag[0],phase[0],d_phase[0],mag[1],phase[1],d_phase[1], ...
        ...
        for (i = 0; i < numbins; ++i) {
            ...
            // Thresholding as Brossier did - discard (ignore) bin's deviation if bin's power is
            // minimal
        ...
        // totdev will be the output, but first we need to fill tempbuf with today's values, ready
        // for tomorrow.
        tbpointer = 0;
        for (i = 0; i < numbins; ++i) {
    ...
    case ODS_ODF_WPHASE:
        ...
        tbpointer = 0;
        for (i = 0; i < numbins; ++i) {
            // Thresholding as Brossier did - discard (ignore) bin's phase deviation if bin's power
            // is low
            if (ods_abs(curr->bin[i].mag) > ods->odfparam) {

                // Deviation is the *second difference* of the phase, which is calc'ed as curval -
                // yesterval - yesterfirstdiff
...
void onsetsds_detect(OnsetsDS *ods) {
    ...
    if (ods->med_odd) {
        ...
        ods->odfvalpost =
            ods->odfvals[0] - ((sortbuf[medspan >> 1] + sortbuf[(medspan >> 1) - 1]) * 0.5f);
    }
